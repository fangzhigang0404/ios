...
- (void)initUI {
    ...
    _pageView.frame = CGRectMake(50, SCREEN_HEIGHT - 80, SCREEN_WIDTH - 100, 20);
...
- (void)requestData {
    ...
    [_loadMoreBtn setTitle:NSLocalizedString(@"just_tip_for_click_load_more", nil)
                  forState:UIControlStateNormal];
    [MPDecorationBaseModel getDataWithParameters:@{@"limit":@(_limit),@"offset":@(_offset)} success:^(NSArray *array) {
        if (array.count == 0) {
        ...
        if (_arrayDS.count == 0) {
            ...
            [_pageView setPageNumWithCount:_arrayDS.count];
