/**
 * @file    MPMyProjectViewController.m
 * @brief   the from of MPMyProjectViewController.
 * @author  Xue
 * @version 1.0
 * @date    2016-01-20
 */
...
{
    UITableView *projectTabelView;    //<! the tableview of listView.
    NSMutableArray *projectArray;     //<! projectArray is the data source of the Bei Shu orders.
    NSString *tableViewType;          //<! order type.
    NSMutableArray *orderArray;       //<! orderArray is the data source of the common orders.
    BOOL isCommon;                    //<! To determine whether ordinary orders.
    NSInteger _beiShuoffset;          //<! _offset offset how many.
    NSInteger _beiShulimit;           //<! _limit limlt how many.
    BOOL _beiShuisLoadMore;           //<! _beiShuisLoadMore load more or not.
    NSInteger _offset;                //<! _offset offset how many.
    NSInteger _limit;                 //<! _limit limlt how many.
    NSInteger _isLoadMore;            //<! _isLoadMore load more or not.
    ...
    [MPMyProjectModel getDataWithParameters:@{@"limit":@(_beiShulimit),@"offset":@(_beiShuoffset)} success:^(NSArray *array) {
       ...
       // if ([IS_BEISHU isEqualToString:@"BEISHU"]) {
            if (projectArray.count == 0) {
                ...
                _emptyView.infoLabel.text = NSLocalizedString(@"just_no_order", nil);
