...
- (void)remove {
    ...
    /// Cancel isCertain must is NO.
    [self.delegate toobarDonBtnHaveClick:self resultString:@"NO" isCertain:NO];
}
- (void)show {
...
- (void)doneClick {
    ...
    if (_pickerView) {
        ...
        _resultString=[NSString stringWithFormat:@"%@",s1];
    }
    
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:isCertain:)]) {
        
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString isCertain:YES];//!< /// Cancel isCertain must is YES.
    }
}
