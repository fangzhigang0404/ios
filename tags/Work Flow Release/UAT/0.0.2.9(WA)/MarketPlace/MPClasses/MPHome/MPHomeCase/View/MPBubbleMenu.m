...
@interface MPBubbleMenu ()
{

    UIBlurEffect *blurEffect;
    UIVisualEffectView *effectView;
    MPBubbleMenu *upMenuView;
}
...
/**
 *  Create button size and button background and button positions.
 *
 *  @return return a view.
 */
- (UIView *)createHomeButtonView{
    
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(106, 136 , 58, 58)];
    UIImageView *addImageview = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 58, 58)];
    addImageview.image = [UIImage imageNamed:SUSPENSION_EXPAND];
    addImageview.layer.cornerRadius = 29;
    
    if (SCREEN_WIDTH > 400) {
        view.frame = CGRectMake(118, 128, 66, 66);
        addImageview.frame = CGRectMake(0, 0, 66, 66);
        addImageview.layer.cornerRadius = 33;
    }
...
- (void)showButtons {
    ...
    self.homeButtonView.backgroundColor = [UIColor clearColor];


    if (imgView==nil) {
        imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 58, 58)];
        imgView.backgroundColor = COLOR(0, 132, 255, 1);
        imgView.image = [UIImage imageNamed:SUSPENSION_CONTRACT];
        ...
        [self.homeButtonView addSubview:imgView];
    }else{
        imgView.image = [UIImage imageNamed:SUSPENSION_CONTRACT];
...
//                
                if (SCREEN_WIDTH > 400) {
                    if (i == buttonContainer.count - 1) {
                        originPosition = CGPointMake(self.frame.size.width/2.f, self.frame.size.height - self.homeButtonView.frame.size.height);
                        finalPosition = CGPointMake(self.frame.size.width/2.f,
                                                    self.frame.size.height - self.homeButtonView.frame.size.height - self.buttonSpacing - button.frame.size.height/2.f
                                                    - ((button.frame.size.height + self.buttonSpacing) * index) - 10);
                    } else {
                        originPosition = CGPointMake(self.frame.size.width/2.f, self.frame.size.height - self.homeButtonView.frame.size.height);
                        finalPosition = CGPointMake(self.frame.size.width/2.f,
                                                    self.frame.size.height - self.homeButtonView.frame.size.height - self.buttonSpacing - button.frame.size.height/2.f
                    ...
                    if (i == buttonContainer.count - 1) {
                        ...
                        finalPosition = CGPointMake(self.frame.size.width/2.f,
                                                    self.frame.size.height - self.homeButtonView.frame.size.height - self.buttonSpacing - button.frame.size.height/2.f
                                                    - ((button.frame.size.height + self.buttonSpacing) * index) - 8);
                    } else {
                        originPosition = CGPointMake(self.frame.size.width/2.f, self.frame.size.height - self.homeButtonView.frame.size.height);
                        finalPosition = CGPointMake(self.frame.size.width/2.f,
                                                    self.frame.size.height - self.homeButtonView.frame.size.height - self.buttonSpacing - button.frame.size.height/2.f
                                                    - ((button.frame.size.height + self.buttonSpacing) * index + 2));
                    }
                }
...
- (void)dismissButtons {
    ...
    imgView.image = [UIImage imageNamed:SUSPENSION_EXPAND];
    imgView.layer.cornerRadius = 29;
    if (SCREEN_WIDTH > 400) {
        imgView.layer.cornerRadius = 33;
    }
