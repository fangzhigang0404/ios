...
- (void)addTap {
    ...
    [self.designerIcom addGestureRecognizer:[[UITapGestureRecognizer alloc]
                                             initWithTarget:self
                                             action:@selector(clickDesignerIcon:)]];
}
...
- (void)clickDesignerIcon:(UITapGestureRecognizer *)tap {
    if ([self.delegate respondsToSelector:@selector(didClickDesignerIocnAtIndex:bidderModel:needModel:)]) {
        [self.delegate didClickDesignerIocnAtIndex:_section bidderModel:_model needModel:_needModel];
    }
}
